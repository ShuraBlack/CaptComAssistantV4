...
import de.shurablack.model.player.EqualizerSettings;
import de.shurablack.model.player.LoadResultHandler;
...
public class Player extends EventWorker {
    ...
    public static EqualizerSettings EQ_SETTINGS;
    ...
    public void processGuildSlashEvent(Member member, MessageChannelUnion channel, String name, SlashCommandInteractionEvent event) {
        ...
        switch (event.getSubcommandName()) {
            ...
            case "position":
                ...
                break;
            case "equalizer":
                String presetName = event.getOption("preset").getAsString();
                float percentage = event.getOption("percentage").getAsInt();
                Consumer<Float> presetFunction = null;
                switch (presetName) {
                    case "bass": presetFunction = MusicManager.applyEq(MusicManager.BASS, musicManager);
                        break;
                    case "mid": presetFunction = MusicManager.applyEq(MusicManager.MID, musicManager);
                        break;
                    case "treble": presetFunction = MusicManager.applyEq(MusicManager.TREBLE, musicManager);
                        break;
                    case "linear": presetFunction = MusicManager.applyEq(MusicManager.LINEAR, musicManager);
                        break;
                }
                EQ_SETTINGS = new EqualizerSettings(presetFunction, percentage);
                Player.EQ_SETTINGS.getPreset().accept(Player.EQ_SETTINGS.getPercentage());
                EmbedBuilder eq = new EmbedBuilder().setDescription(String.format("%s, Preset wurde auf **%s (%.0f%%)** gesetzt"
                        , member.getAsMention(), presetName, percentage));
                event.replyEmbeds(eq.build()).setEphemeral(true).queue();
                break;
        }
    }
    ...
    public void processPublicReactionEvent(Member member, MessageChannelUnion channel, String name, MessageReactionAddEvent event) {
        if (!event.getMessageId().equals(LocalData.getMessageID("player_main"))) {
            return;
        }
        ...
        switch (textID) {
            case "player link":
                ...
                DiscordBot.getAudioPlayer().loadItemOrdered(musicManager,
                        url, new LoadResultHandler(musicManager, member, channel.asTextChannel(), url, event, false));
                break;
            case "player search":
                String query = event.getValue("query").getAsString();
                DiscordBot.getAudioPlayer().loadItemOrdered(musicManager,
                        "ytsearch:" + query, new LoadResultHandler(musicManager, member, channel.asTextChannel(), query, event, true));
                break;
        }
    }
    ...
    public void addReactions (TextChannel channel) {
        String mesID = LocalData.getMessageID("player_main");
